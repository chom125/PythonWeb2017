...
def server(log_buffer=sys.stdout):
    ...
    print("making a server on {0}:{1}".format(*address), file=log_buffer, flush=True)
    ...
    try:
        ...
        while True:
            print('waiting for a connection', file=log_buffer, flush=True)
            ...
            try:
                print('connection - {0}:{1}'.format(*addr), file=log_buffer, flush=True)
                ...
                conn.close()
                print(
                    'echo complete, client connection closed', file=log_buffer, flush=True
                )
        ...
        sock.close()
        print('quitting echo server', file=log_buffer, flush=True)
